...
- Автоматически выбранный KGP добавляется в buildscript classpath корневого проекта, чтобы встроенный Kotlin в AGP 9 фактически использовал этот компилятор и поддерживаемые им JVM target, а не более старую встроенную версию.
...
Settings-плагин автоматически добавляет выбранный KGP в buildscript classpath корневого проекта; при использовании встроенного Kotlin в AGP 9 не применяйте дополнительно `org.jetbrains.kotlin.android`. Результат выбора можно прочитать как объект через `gradle.extra["platformVersions"]`.
...
Ежедневный workflow обновляет и проверяет данные, создаёт коммит и тег корректирующего выпуска только при семантических изменениях, а затем запускает защищённую цепочку публикации в двух реестрах и GitHub Release; ручные режимы check и update-pr остаются доступными:
...
* `Улучшение` Обновлены встроенные данные о совместимости платформ и выпусках из официальных первичных источников; перед публикацией плановая автоматизация проверила разбор сборщиками, поведение плагина Gradle и сборку потребителя без IDE

# v1.7.3

###### 2026/09/03

* `Исправление` Settings-плагин теперь добавляет автоматически выбранный KGP в корневой buildscript classpath до разрешения плагинов проекта; встроенный Kotlin в AGP 9 больше не остаётся на комплектном KGP 2.2.10 и не отклоняет JVM target 25 при JDK 25
* `Улучшение` Добавлена проверка AGP-потребителя без явно запрошенного Kotlin-плагина, подтверждающая совпадение выбранного KGP с фактически разрешённой версией в корневом classpath; Accessibility Compat проверен с Gradle 9.5/AGP 9.3.2 на JDK 25 и 26 с модульными тестами, lint, сборками APK и 28 тестами на четырёх устройствах

# v1.7.2

###### 2026/09/03

* `Исправление` Идентификационные свойства Android Studio, передаваемые через Gradle `-P`, объединены с системными свойствами JVM, поэтому Quail 3 больше не сокращается до `2026.1`, не выбирает по ошибке AGP 9.2.1 и не отклоняет автоматически выбранную цель JVM 25
* `Улучшение` Добавлена бинарно совместимая перегрузка Facade для явной передачи свойств проекта Gradle и резервное определение версии strict для ещё не собранных build IDE; проверено, что Quail 3 с Gradle 9.5/9.7 и JDK 25/26 выбирает AGP 9.3.2 и успешно создаёт задачи Kotlin/KSP
