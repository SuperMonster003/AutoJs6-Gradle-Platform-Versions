...
- IDE 環境使用平台對應表決定上限並保留落後回退；Temurin 與裸命令列直接採用 Gradle 相容上限。
...
值為 `NONE` 或留空時表示不固定。若只想宣告下限而不鎖定版本，可使用 `MIN_SUPPORTED_ANDROID_GRADLE_PLUGIN_VERSION`；數字形式的 `COMPILE_SDK_VERSION` 與 `TARGET_SDK_VERSION` 會自動納入判斷.
